/*
        ...
        Please, do not bother this, unless you know what it does - this took TOO
   long to write :sob:
*/
...
const Functions = {
  ...
  sendAPIRequest: async (url, headers) => {
    ...
    try {
      const response =
          await fetch(`https://backend.camellia.wiki/${url}`, {headers});
      if (!response.ok) {
  ...
  convertTimestamp: (timeStamp, format = 'MM/DD/YYYY HH:II:SS') => {
    ...
    return format.replace(
        /mm|MM|m|M|dd|DD|yyyy|YYYY|yy|YY|hh|HH|ii|II|ss|SS/g,
        match => formats[match]);
  }
};
...
const updateUserData = async () => {
  ...
  setData = () => {
    ...
    const profileHeader =
        profileCard.querySelector('#profile-details #profile-header');
    const profileSubHeader =
        profileCard.querySelector('#profile-details #profile-subheader');
    ...
    profileCard.style.setProperty(
        '--profile-accent', user.color ? user.color : defaults.color);
    profileHeader.innerText = user.name ? user.name : defaults.name;
    // TODO: Once we check how many comments by the user has made been in the
    // API, we use it here: profileSubHeader.innerText =
    profileSubHeader.innerText = user.name ?
        `Joined ${
            Functions.convertTimestamp(
                (user.join * 1000), 'M/DD/yy')} - 0 Comments` :
  ...
  if (dh) {
    ...
    Functions.Cookie.set('wiki_auth', JSON.stringify({dh, user: data.data}), 7);
...
loginBtn.addEventListener('click', (event) => {
  ...
  if (loginBtn.querySelector('i').className == 'ph-bold ph-sign-out') {
    ...
    const popupParams =
        'scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,width=833,height=654';
    popupWindow = window.open(
        `https://discord.com/oauth2/authorize?response_type=token&client_id=${
            DISCORD_CLIENT_ID}&scope=identify&redirect_uri=${
            window.location.protocol}//${window.location.host}/oauthComplete`,
        'popup', popupParams);
    popupWindow.focus();

    const popupMsgAlert = setInterval(() => {
      popupWindow.postMessage(
          '', `${window.location.protocol}//${window.location.host}/`);
    }, 500);

    // This stupid thing will wait for the oauth complete page to send back our
    // required data :)
    window.addEventListener('message', async (event) => {
      if (popupWindow && !popupWindow.closed && event.data.token) {
        ...
        Functions.Cookie.set(
            'wiki_auth', JSON.stringify({dh: event.data.token, user: {}}), 7);
...
// Attempt to laod in user data once loaded (this requires the script to be
// executed last in the body)
updateUserData();