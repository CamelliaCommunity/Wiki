...
class ToastManager {
    ...
    push(obj) {
        ...
        toast.innerHTML = `
            ...
            ${obj.closeButton == null || obj.closeButton === true ? '<button class="toast-notification-close"><i class="ph ph-x"></i></button>' : ''}
        `;
		...
		if (this.options.autoCloseToasts) {
			if (this.options.autoCloseToasts.enabled && !this.stack[index-1].timer)
				this.stack[index-1].timer = setTimeout(() => { this.closeToast(this.stack[index-1]); }, this.options.autoCloseToasts.afterDuration);

			toast.addEventListener("mouseover", (event) => {
				if (this.options.autoCloseToasts.enabled && this.stack[index-1].timer) {
					clearTimeout(this.stack[index-1].timer);
					this.stack[index-1].timer = null
				};
			});
    ...
    openToast(toast) {
        ...
        toast.element.addEventListener('transitionend', () => {
			// toast.element.style.transform = `translate(-20px, ${toast.offsetY}px)`;
			// setTimeout(() => {
