  ...
  (function (window) {
    ...
    function registerInput () {
      options.searchInput.addEventListener('input', function (e) {
        if (isWhitelistedKey(e.which)) {
          ...
          options.searchWrapper.style.display = "flex";
        ...
        function hideResults(el) {
          ...
          let targetInside = options.searchWrapper.contains(el.target) || options.searchInput.contains(el.target);
		  
          if (!targetInside) {
            options.searchWrapper.style.display = "none";  
          }	
          else {
            return;
          }        
    ...
    function render (results, query) {
      ...
      for (let i = 0; i < len; i++) {
		...
		let tmpCategory = options.searchCategoryOverrides[result.searchCategory] || result.searchCategory;
		if (options.shouldSortCategory && currentCategory !== tmpCategory) {
			currentCategory = tmpCategory;
			appendToResultsContainer(`<h3>${currentCategory}</h3>`);
		};
